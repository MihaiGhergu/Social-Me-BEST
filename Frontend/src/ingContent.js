...
export class ingContent extends LitElement {
  ...
  getContent() {
    ...
    switch (this.content_value) {
      case 0:
        return html` <h1>Please enter your account number and PIN</h1> `;
      case 1:
        return html` <h1>Please choose an option below</h1> `;
      case 2:
        return html`
      ...
      case 4:
        ...
        return html`
          ...
          <ol>
            ${this.transactions.map(
              i =>
                html`<ing-transaction
                  @click="${this.toggleShow}"
                  accountNo=${accno}
                  transactionInfo=${JSON.stringify(i)}
                >
                </ing-transaction>`
            )}
          </ol>
        `;
      case -1:
        return html` <h1>Error at login</h1> `;
    }
  }

  toggleShow(event) {
    if (event.target.showSelf == true) event.target.showSelf = false;
    else event.target.showSelf = true;
  }
  ...
  async _onTransactionSubmit(event) {
    ...
    if (response.status == 201) this.dispatchEvent(new CustomEvent('message-sent', { detail: 1 }));
    else this.dispatchEvent(new CustomEvent('message-sent', { detail: 0 }));
  }
